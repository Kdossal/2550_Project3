...
simulate_NHANES <- function(){
  ...
  sim_data <- as.data.frame(
                    mvrnorm(3000, # Number of observations in NHANES 
                      mu = rep(0,8), 
                      Sigma = cor_matrix)) %>% 
            mutate_all(pnorm)
  
  # Assign Sex first, since statistics for NHANES are stratified by sex
  sim_data$SEX <- c(rep(1,1400),rep(2,1600)) # SEX is binary 1/2
  ...
  sim_data$AGE <- ifelse(
    sim_data$SEX == 1,
    quantile(sdz_AGE, sim_data$AGE)*9.37 + 46.88,
    quantile(sdz_AGE, sim_data$AGE)*9.25 + 46.61
  )
...
ggplot_hist <- ggplot(data.frame(brier_risks_men), aes(x = brier_risks_men)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.002, fill = "lightblue", color = "black") + 
  geom_vline(aes(xintercept = mean(nh_brier_risks_men)), color = "red", linetype = "dashed") +
  annotate("text", x = mean(brier_risks_men), y = 150, 
           label = "Non-Simulated Brier Score", color = "red", hjust = 1.55) +
  ggtitle("Men's Brier Risk Estimate Distribution") +
  xlab("Brier Risk Estimates") +
  ylab("Density") +
  theme_minimal()
...
```{r}
# Create the histogram using ggplot
ggplot_hist <- ggplot(data.frame(brier_risks_women), aes(x = brier_risks_women)) +
  geom_histogram(aes(y = ..density..), binwidth = 0.0007, fill = "lightblue", color = "black") + 
  geom_vline(aes(xintercept = mean(nh_brier_risks_women)), color = "red", linetype = "dashed") +
  annotate("text", x = mean(nh_brier_risks_women), y = 400, 
           label = "Non-Simulated Brier Score", color = "red", hjust = -.1) +
  ggtitle("Women's Brier Risk Estimate Distribution") +
  xlab("Brier Risk Estimates") +
  ylab("Density") +
  theme_minimal()
